package at.feddis08.tools.remote_interface.server.main;
...
import at.feddis08.tools.remote_interface.server.socket.Client;
        ...
        while (index < Server.clients.size()) {
            ...
            while (index2 < client.requests.size()) {
                ...
                if (Objects.equals(command[0], "reload_scripts")) {
                    if (client.player.logged_in){
                        PlayerObject dbPlayer = Functions.getPlayer("id", client.player.id);
                        if (Rank_api.has_permission_from_rank(dbPlayer.player_rank, "do_reload_scripts") || Rank_api.has_permission_from_rank(dbPlayer.player_rank, "*")){
                            Boot.consoleLog("Do reload for all scripts by: " + dbPlayer.display_name + ":" + dbPlayer.id);
                            Main.reload_all();
                        }
                    }
                }
                if (Objects.equals(command[0], "reload_server")) {
                    if (client.player.logged_in){
                        PlayerObject dbPlayer = Functions.getPlayer("id", client.player.id);
                        if (Rank_api.has_permission_from_rank(dbPlayer.player_rank, "do_reload_server") || Rank_api.has_permission_from_rank(dbPlayer.player_rank, "*")){
                            Boot.consoleLog("Do reload by: " + dbPlayer.display_name + ":" + dbPlayer.id);
                ...
                if (Objects.equals(command[0], "post_file")) {
                    if (client.player.logged_in){
                        PlayerObject dbPlayer = Functions.getPlayer("id", client.player.id);
                        if (Rank_api.has_permission_from_rank(dbPlayer.player_rank, "do_post_file") || Rank_api.has_permission_from_rank(dbPlayer.player_rank, "*")){
                ...
                if (Objects.equals(command[0], "get_file")) {
                    if (client.player.logged_in){
                        PlayerObject dbPlayer = Functions.getPlayer("id", client.player.id);
                        if (Rank_api.has_permission_from_rank(dbPlayer.player_rank, "do_get_file") || Rank_api.has_permission_from_rank(dbPlayer.player_rank, "*")){
                ...
                if (Objects.equals(command[0], "get_server_info")) {
                    if (client.player.logged_in){
                        PlayerObject dbPlayer = Functions.getPlayer("id", client.player.id);
                        if (Rank_api.has_permission_from_rank(dbPlayer.player_rank, "do_get_server_info") || Rank_api.has_permission_from_rank(dbPlayer.player_rank, "*")){
                ...
                if (Objects.equals(command[0], "get_list_files")) {
                    if (client.player.logged_in){
                        PlayerObject dbPlayer = Functions.getPlayer("id", client.player.id);
                        if (Rank_api.has_permission_from_rank(dbPlayer.player_rank, "do_get_list_file") || Rank_api.has_permission_from_rank(dbPlayer.player_rank, "*")){
                            File f = new File(command[1]);
