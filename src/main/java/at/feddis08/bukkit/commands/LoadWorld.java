...
import at.feddis08.bukkit.minecraft.tools.StartLoadWorld;
import org.bukkit.*;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("loadWorld")) {
            ...
            if (Objects.equals(dbPlayer.didStartup, "true")) {
                if (args.length == 1) {
                    try {
                        if (Rank_api.isPlayer_allowedTo(dbPlayer.id, "doLoadWorld") || Rank_api.isPlayer_allowedTo(dbPlayer.id, "*")) {
                    ...
                    try {
                        if ((Rank_api.isPlayer_allowedTo(dbPlayer.id, "doLoadWorld") && Rank_api.isPlayer_allowedTo(dbPlayer.id, "doSetAutoloadWorld")) || Rank_api.isPlayer_allowedTo(dbPlayer.id, "*")) {
                            sender.sendMessage("Loading world: " + args[0]);
